...
1. The visible reasoning trace can change without corresponding changes in the final answer, suggesting it may not directly expose the internal causal process.
2. In some experiments, models remained surprisingly capable even when intermediate reasoning tokens were modified, removed, or disrupted.
3. These domains offer structured problems and answers that can often be objectively verified, making training and evaluation easier.
4. Human-like language can tempt readers to assume the model uses human-like mental processes even when the underlying mechanism may be very different.
